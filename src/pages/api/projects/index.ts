...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createProject() {
    ...
    const body = { ...req.body };
    if (body?.client_project_access?.length > 0) {
      const create_client_project_access = body.client_project_access;
      body.client_project_access = {
        create: create_client_project_access,
      };
    } else {
      delete body.client_project_access;
    }
